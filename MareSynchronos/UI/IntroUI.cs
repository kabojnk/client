...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    public IntroUi(ILogger<IntroUi> logger, UiSharedService uiShared, MareConfigService configService,
        ...
        PerformanceCollectorService performanceCollectorService, DalamudUtilService dalamudUtilService) : base(logger, mareMediator, "Cyprus Synchronos Setup", performanceCollectorService)
    {
        _uiShared = uiShared;
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to Cyprus Synchronos");
            ImGui.Separator();
            UiSharedService.TextWrapped("Cyprus Synchronos is a plugin that will replicate your full current character state including all Penumbra mods to other paired Mare Synchronos users. " +
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, Cyprus Synchronos will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where Cyprus Synchronos will download other character files to. " +
                                     "Once the storage folder is set and the scan complete, this page will automatically forward to registration at a service.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of Cyprus Synchronos in the Plugin Configurations folder of Dalamud. " +
        ...
        else if (!_uiShared.ApiController.ServerAlive)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use Cyprus Synchronos you will have to register an account.");
            UiSharedService.TextWrapped("For the official Cyprus Synchronos Servers the account creation will be handled on the official Mare Synchronos Discord. Due to security risks for the server, there is no way to handle this sensibly otherwise.");
            ...
            if (ImGui.Button("Join the Cyprus Synchronos Discord"))
            {
